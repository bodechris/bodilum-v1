...
export async function GET() {
  ...
  return NextResponse.json({ status: ready ? "ok" : "degraded", services: { googlePlaces: Boolean(env.googlePlacesApiKey), persistentRateLimit: Boolean(env.mongoUri), profileStorage: Boolean(env.mongoUri), bedrock: bedrockConfigured(), searchEnabled: env.prospectSearchEnabled, analysisEnabled: env.prospectAnalysisEnabled, brandScorecardEnabled: env.brandScorecardEnabled, demoMode: env.demoMode }, ...(process.env.NODE_ENV !== "production" ? { issues } : {}) }, { status: ready ? 200 : 503 });
}
